...
differences_output = cv2.VideoWriter('../videos/differences_ii.avi', codec, fps, frame_size, isColor)
# same for the full video
output = cv2.VideoWriter('../videos/encoded_ii.avi', codec, fps, frame_size, isColor)
...
errors_bitstream_file = open('../binary_files/error_frames.bin', 'wb')
motion_bitstream_file = open('../binary_files/motion_vectors.bin', 'wb')
...
while capture.isOpened():
    ...
    if frame_idx % 12 != 0:
        ...
        motion_vectors, difference_error = motionCompensation(search_radius_k, macroblock_size, reference_frame, previous_frame, 'exhaustive')
        differences_output.write(cv2.cvtColor(difference_error.astype(np.uint8), cv2.COLOR_RGB2BGR))
        ...
        output.write(cv2.cvtColor(previous_frame, cv2.COLOR_RGB2BGR))
        ...
        output.write(cv2.cvtColor(reference_frame, cv2.COLOR_RGB2BGR))
...
writeHuffmanCodes(motion_vectors_codes_dict, '../binary_files/motion_vectors_codes.pkl')
writeHuffmanCodes(error_images_dict, '../binary_files/error_images_codes.pkl')
