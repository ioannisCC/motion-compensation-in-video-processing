...
motion_vectors_codes_dict = readHuffmanCodes('../binary_files/motion_vectors_codes_log.pkl')
error_images_dict = readHuffmanCodes('../binary_files/error_images_codes_log.pkl')

# open bitstream file
with open('../binary_files/error_frames_log.bin', 'rb') as bitstream_file_frames:
    with open('../binary_files/motion_vectors_log.bin', 'rb') as bitstream_file_motion_vectors:

        capture = cv2.VideoCapture('../videos/encoded_iii.avi')
        ...
        # set the output
        output = cv2.VideoWriter('../videos/decoded_iii.avi', codec, fps, frame_size, isColor)
        ...
        while True:
            ...
            reconstructed_frame_rgb = cv2.cvtColor(reconstructed_frame.astype(np.uint8), cv2.COLOR_RGB2BGR)
            # add decoded to output
            output.write(reconstructed_frame_rgb)
            previous_frame = reconstructed_frame_rgb.copy()
            frame_idx += 1
